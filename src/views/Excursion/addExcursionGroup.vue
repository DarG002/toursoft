    ...
    el-main
      el-form(label-width="200px")
        ...
        el-form-item(label="Время")
          ...
          size="small")
        el-form-item
          el-button(type="primary", @click.stop="addExcursionGroup") Add
</template>

<script>
  import { hotel, excursionGroup } from '@/api'
  ...
  export default {
    ...
    methods: {
      addExcursionGroup() {
        ...
        excursionGroup({ method: 'post', data: { userId: getUserId(), excursionId: this.data.excursionId, person: this.person } }).then(_ => {
          this.data.cb()
        }).catch(err => {
          throw new Error(err)
        })
